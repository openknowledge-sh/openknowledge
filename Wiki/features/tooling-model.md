...
| Experimental local agents and automation | `agent`, `jobs` | Edit the current filesystem through a steered Codex, Claude Code, Grok, or OpenCode session, execute setup/source workflows, opt into an isolated worktree, or run strict per-runtime scheduled jobs that are always isolated. |
