...
The shipped Docker targets are `serve`, `publisher`, `worker-codex`,
`worker-claude`, and `worker-opencode`. They use separate users, volumes,
secrets, read-only roots, capability drops, and no Docker socket. Only `serve`
has public ingress. On providers without shared volumes, authenticated private
HTTP endpoints carry artifacts and bundle exchange with distinct capabilities.
