...
the project-owned `.openknowledge/runtime/Dockerfile` generated by
`openknowledge deploy railway init`; one entrypoint selects the service role,
while Railway still scopes ingress, volumes, and credentials per service. Only
`serve` has public ingress. On providers without shared volumes, authenticated
private HTTP endpoints carry artifacts and bundle exchange with distinct
capabilities.
