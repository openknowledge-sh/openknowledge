...
| [§6 Index Files](../SPEC.md#6-index-files), [§9 Conformance](../SPEC.md#9-conformance), and [§11 Versioning](../SPEC.md#11-versioning) | Reserved `index.md` files must follow index-file structure when present; frontmatter is only permitted in root `index.md` for `okf_version`. | ✅ Compliant | Root `index.md` frontmatter may only declare `okf_version`, non-root indexes must not use frontmatter, and CLI extension metadata now lives in `openknowledge.toml`. | [TestValidateReservedFiles](https://github.com/openknowledge-sh/openknowledge/blob/main/packages/cli/internal/okf/validate_test.go); [TestValidateRootIndexRejectsBundleMetadata](https://github.com/openknowledge-sh/openknowledge/blob/main/packages/cli/internal/okf/validate_test.go); [TestValidateIndexRejectsPublishMetadata](https://github.com/openknowledge-sh/openknowledge/blob/main/packages/cli/internal/okf/validate_test.go); [validateIndex](https://github.com/openknowledge-sh/openknowledge/blob/main/packages/cli/internal/okf/validate.go) |
...
No known ❌ blocking conflict with OKF v0.1 section 9 is documented here, and
no yellow hard-rule rows remain for OKF v0.1.
