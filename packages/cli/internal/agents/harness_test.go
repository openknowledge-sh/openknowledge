...
func TestHarnessCommandsUseCanonicalHeadlessContracts(t *testing.T) {
	tests := []struct {
		...
		{runtime: RuntimeGrok, model: "grok-4.5", command: "grok", args: []string{"--no-auto-update", "--always-approve", "--model", "grok-4.5", "--single"}, promptMode: PromptArgument},
		{runtime: RuntimeOpenCode, model: "xai/grok-test", command: "opencode", args: []string{"run", "--auto", "--model", "xai/grok-test"}, promptMode: PromptArgument},
	}
...
func TestHarnessRegistryRejectsUnknownRuntime(t *testing.T) {
	if _, err := HarnessForRuntime("grok-cli"); err == nil || !strings.Contains(err.Error(), "claude, codex, grok, opencode") {
