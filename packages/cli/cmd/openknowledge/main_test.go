...
func TestHelpTextOrganizesCommandsAroundProductWorkflows(t *testing.T) {
	...
	required := []string{
		...
		"check        Run configured knowledge checks and report one status.",
		"search       Search managed or ordinary Markdown with source context.",
		"review       Review knowledge content with an optional agent.",
		"publish      Publish checked viewer and MCP artifacts.",
		"upgrade      Upgrade a knowledge base to a supported OKF version.",
	...
	forbidden := []string{
		...
		"openknowledge automation insights create \"Document the deployment rollback workflow\"",
		"openknowledge automation deploy railway Wiki --dry-run",
		"\n  jobs         ",
		"\n  insights     ",
		"\n  runtime      ",
		"\n  deploy       ",
...
func TestCommandHelpTextIncludesCommandSpecificDetails(t *testing.T) {
	tests := map[string]struct {
		...
		"setup": {
			...
			required: []string{
				"openknowledge setup\n",
				"openknowledge setup [wiki] --from <directory> --mode copy",
		...
		"check": {
			...
			required: []string{
				...
				"Audit, eval, claims, and publication checks",
			},
		},
		"publish": {
			help: publishHelpText(),
			required: []string{
		...
		"review": {
			...
			required: []string{
				...
				"optionally run an installed agent",
				"do not change deterministic check status",
			},
		},
		"prompt review rules": {
		...
		"jobs": {
			...
			required: []string{
				...
				"Experimental command group for deterministic local jobs",
				"may still",
			},
		},
		"jobs new": {
			help: jobsNewHelpText(),
			required: []string{
