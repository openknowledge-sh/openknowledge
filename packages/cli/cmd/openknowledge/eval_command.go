...
func printEvalReport(report knowledgeeval.Report, options evalRunOptions) error {
	if options.format == "markdown" {
		return writeEvalOutput(options.out, []byte(renderEvalMarkdown(report)), "eval report")
	}
...
func printEvalComparison(report knowledgeeval.ComparisonReport, options evalRunOptions) error {
	if options.format == "markdown" {
		return writeEvalOutput(options.out, []byte(renderEvalComparisonMarkdown(report)), "eval comparison")
	}
	if options.format == "json" {
		content, err := json.MarshalIndent(report, "", "  ")
		if err != nil {
...
func renderEvalMarkdown(report knowledgeeval.Report) string {
...
func renderEvalComparisonMarkdown(report knowledgeeval.ComparisonReport) string {
	var output strings.Builder
...
func renderCaseMarkdown(output *strings.Builder, result knowledgeeval.CaseResult) {
...
func renderAnswerMarkdown(output *strings.Builder, label string, answer *knowledgeeval.AnswerResult) {
...
func renderChecksMarkdown(output *strings.Builder, result knowledgeeval.CaseResult) {
	failed := 0
	for _, check := range result.Checks {
		if !check.Passed {
			failed++
